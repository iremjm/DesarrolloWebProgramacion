...
public class GarajeCoches {
		...
		public void  entra (Vehiculos v){
			for(int i=0;i<MAX_PLAZAS;i++){
				if(plazas[i]== null){
					plazas[i]=v;
					return;//sale fuera de la función con un return, con un break sale fuera del for, while o dowhile
				}//if	
		...
		//Metodo devuelve el vehículo que se encuentra en dicha plaza
		...
		//contador para saber donde meter el índice de la plaza libre
			int contList=0;
			for(int i=0;i<MAX_PLAZAS;i++){
				if(plazas[i]==null){
					lista[contLsita]=i;
